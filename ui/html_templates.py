...
def create_insight_list(items: list[str]) -> str:
    ...
    items_html = "".join(INSIGHT_ITEM_TEMPLATE.format(content=html.escape(item)) for item in items)
...
def create_topic_section(title: str, content: str) -> str:
    ...
    Note: title is escaped for XSS prevention, but content is assumed to be
    pre-formatted HTML (e.g., from LLM streaming or JSON dumps) and is NOT escaped.
    """
    return TOPIC_TEMPLATE.format(title=html.escape(title), content=content)
...
def create_error_message(message: str) -> str:
    ...
    return ERROR_TEMPLATE.format(message=html.escape(message))


def create_status_message(message: str) -> str:
    """Create a status message display."""
    return STATUS_TEMPLATE.format(message=html.escape(message))
